...
if kubectl rollout status deployment/order-deployment -n prod | grep -q "successfully rolled out"; then
  echo "Success to create new kubernetes deployment resources"

  kubectl annotate deployment/order-deployment kubernetes.io/change-cause="Change image to ${IMAGE_TAG}"
  ...
  kubectl rollout undo deployment/order-deployment -n prod
  if kubectl rollout status deployment/order-deployment -n prod | grep -q "successfully rolled out"; then
    echo "Rollback was successful"

    kubectl annotate deployment/order-deployment kubernetes.io/change-cause="Rollback to previous image"
  else
